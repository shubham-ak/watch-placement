...
export function init() {
    ...
    renderer.domElement.style = "display: block; width: 640px; height: 480px; position: absolute; left: -11px; top: 0;"
...
export function updateLandmarkPoints(newLandmarkPoints) {
    ...
    if (landmarkPoints.length > 0) {
        ...
        let wristX = ((landmarkPoints[4].x / 1) * 2) - 1;
        let wristY = -((landmarkPoints[4].y / 1) * 2) + 1;
        let wristZ = ((landmarkPoints[4].z / 1) * 2) - 1;

        let pinkyX = ((landmarkPoints[5].x / 1) * 2) - 1;
        let pinkyY = -((landmarkPoints[5].y / 1) * 2) + 1;
        let pinkyZ = ((landmarkPoints[5].z / 1) * 2) - 1;


        let indexX = ((landmarkPoints[6].x / 1) * 2) - 1;
        let indexY = -((landmarkPoints[6].y / 1) * 2) + 1;
        let indexZ = ((landmarkPoints[6].z / 1) * 2) - 1;
        ...
        // sphere.applyQuaternion(quaternion_z);
        // axesHelper.applyQuaternion(quaternion_z);

        //console.log("Axis z : ", (zWatchOrientation - prevZWatchOrientation));
